...
Elige una opción:  ------->  ''')
        ...
        if Menu == '1':
            ...
            send = Environment(loader=FileSystemLoader('F:\Bibliotecas\Documents\U\2022\Primer Semestre\Lab Lenguajes\Proyectos\LFP_PR_202001568\Plantilla'),
                                autoescape =select_autoescape(['html']))
            
            template = send.get_template('plantilla.html')

            html_file = open('Reporte.html', 'w+', encoding='utf-8')
            # html_file.write(template.render(masvendido = MasVendido, menosvendido = MenosVendido))
            html_file.close()
